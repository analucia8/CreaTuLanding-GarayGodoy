...
export default function ItemCount({ stock = 0, initial = 1, onAdd }) {
  // clamp del initial dentro de [1, stock] (si hay stock)
  const clampInitial = () => {
    if (stock <= 0) return 0;
  ...
  // si cambia el stock desde el detalle, reajustamos qty
  useEffect(() => {
    setQty(clampInitial());
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
      ...
      <span style={{ minWidth: 28, textAlign: "center", color: "#fff" }}>{qty}</span>
      <button onClick={inc} disabled={qty >= stock}>＋</button>
      <button onClick={handleAdd} disabled={stock === 0}>
        Agregar
      </button>
      <small style={{ color: "#ddd" }}>(stock: {stock})</small>
    </div>
  );
}
